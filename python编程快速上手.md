...
##第2章 控制流
###2.1
Python 基础 . 3 在交互式环境中输入表达式 .3
整型、浮点型和字符串数据类型.6 字符串连接和复制6
在变量中保存值7
###1.4.1 赋值语句 . 7
###1.4.2 变量名 . 9
##第一个程序 9
...
导入模块 40 from import 语句. 41
用 sys.exit()提前结束程序 .41 小结.41
习题 .41 函数.43 def 语句和参数44 返回值和 return 语句 45 None 值.46 关键字参数和 print() 47 局部和全局作用域48
###3.5.1 局部变量不能在全局作用 域内使用 . 48
###3.5.2 局部作用域不能使用其他 局部作用域内的变量 . 49
###3.5.3 全局变量可以在局部作用 域中读取 . 49
###3.5.4 名称相同的局部变量和全局 变量 . 50
###4.2
###4.3 4.4
###4.1.5 用下标改变列表中的值 .62
###4.1.6 列表连接和列表复制 .62
...
###4.2.3 多重赋值技巧 .66
增强的赋值操作 . 66 方法  67
###4.4.1 用 index()方法在列表中 查找值 .67
###4.4.2 用 append()和 insert()方法在 列表中添加值 .68
###4.4.3 用 remove()方法从列表中 删除值 .69
###4.4.4 用 sort()方法将列表中的值 排序 .69
例子程序:神奇 8 球和列表  70 类似列表的类型:字符串和
元组 71
###4.6.1 可变和不可变数据类型 .72
###4.6.2 元组数据类型 .73
...
###4.10.1 逗号代码 .79
###4.10.2 字符图网格 .79
字典和结构化数据 .81 字典数据类型 . 81
###3.6
###3.7
###3.8
global 语句 .50 异常处理 52 一个小程序:猜数字 .54 小结.55
###4.5
###4.6
###4.7
###4.8
